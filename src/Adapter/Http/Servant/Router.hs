...
import qualified Adapter.Http.Servant.LoginUser
                                               as Handler
import qualified Adapter.Http.Servant.RegisterUser
                                               as Handler



start :: (MonadIO m, UC.Logger m, MonadThrow m) => Lib.Router m
start logicHandler runner = pure $ serve (Proxy :: Proxy API) $ hoistServer
  (Proxy :: Proxy API)
  (ioToHandler . runner)
  (server logicHandler)
...
server
  :: (MonadThrow m, MonadUnliftIO m, MonadIO m, UC.Logger m) => UC.LogicHandler m -> ServerT API m
